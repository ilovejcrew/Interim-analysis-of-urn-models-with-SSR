...
#ssrfl is the indicator for whether we will combine the SSR in the simulation. Here we offer flexibility to the code to avoid SSR.
...
for  ( i in 1:m ){
        ...
        N1=0            # number of patients go to treatment 1
        N2=0            # number of patients go to treatment 2
        ...
        for (j in 1:n1){
            ...
            x<-runif(1,0,1)	
                ...
                ball1=ball1+sqrt(p1hat) #sqrt(p1hat) balls of type 1 are added to the urn
         	      ball2=ball2+sqrt(p2hat) #sqrt(p2hat) balls of type 2 are added to the urn
		                     }
        
        p1hat=(sum(xx1)+1)/(N1+1)
        p2hat=(sum(xx2)+1)/(N2+1)
        
        stat=abs((p1hat-p2hat)/sqrt(p1hat*(1-p1hat)/N1+p2hat*(1-p2hat)/N2))	#calculate the sequential test statistics 
...
#stat30 is the original test statistics for stage 3, and stat3 is the our new proposed test statistics for stage 3
       ...
       rho1=c(rho1,N1/(N1+N2))  #allocation proportion of treatment 1
       rho2=c(rho2,N2/(N1+N2))  #allocation proportion of treatment 2
